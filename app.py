...
# ==========================================
# 3. 儀表板顯示邏輯
@@ -109,132 +123,130 @@ def clean_num(x):
...
buffer_cash = buffer_cash_df['金額'].sum() 
    ...
    # B. 既有現金 (General Cash) - 優先動用
normal_cash_df = assets_df[(assets_df['類別'] == '現金') & (assets_df['備援'] == False)]
normal_cash = normal_cash_df['金額'].sum()

    # Total Assets
    # C. 投資資產 (股票)
    invest_assets_df = assets_df[(assets_df['類別'] != '現金') & (assets_df['備援'] == False)]
    invest_assets = invest_assets_df['金額'].sum()
    
    # D. 一般投資總資產 (GK基數) = 既有現金 + 股票
    general_assets = normal_cash + invest_assets
...
col1.metric("真實總資產", f"${total_assets/10000:,.0f} 萬", 
                ...
                help=f"股票 {invest_assets/10000:.0f}萬 + 既有現金 {normal_cash/10000:.0f}萬 + 備援現金 {buffer_cash/10000:.0f}萬")

col2.metric("總負債", f"${total_liabilities/10000:,.0f} 萬", delta_color="inverse")
    
col3.metric("淨資產", f"${net_worth/10000:,.0f} 萬")
    col4.metric("🛡️ 抵利型備援現金", f"${buffer_cash/10000:,.0f} 萬", delta="最後防線", delta_color="off")
    ...
    st.info(f"💰 **現金水位分析**：既有活存 **${normal_cash/10000:,.0f} 萬** (優先) / 抵利型備援 **${buffer_cash/10000:,.0f} 萬** (最後)")

st.markdown("---")

    # --- 3. 核心功能：四層水位資金調度 ---
    # --- 3. 核心功能：GK + 股息 + 優先扣現金 + 最後備援 ---
st.header("🌊 現金流與提領策略")
...
sell_stock_amount = max(0, gk_total_limit - dividend_income) 
    ...
    # [計算缺口]
    # 第一階段：股息 + 賣股 是否夠付支出？
    income_stage1 = dividend_income + sell_stock_amount
    gap_stage1 = total_expense - income_stage1
...
# 如果缺口小於既有現金，就只扣一部分；否則把既有現金扣光
        ...
        use_normal_cash = min(gap_stage1, normal_cash)

        # 計算缺口 2 (看還夠不夠)
        gap_2 = gap_1 - use_normal_cash
        # 剩下還不夠的
        remaining_gap = gap_stage1 - use_normal_cash
...
st.subheader("📊 資金調度順序")
        ...
        st.metric("1. 股息收入", f"${dividend_income:,.0f}")
        st.metric("2. GK 賣股 (若需要)", f"${sell_stock_amount:,.0f}")

if use_normal_cash > 0:
            st.metric("3. 動用既有現金", f"${use_normal_cash:,.0f}", delta="Layer 3 (優先)", delta_color="inverse")
            st.metric("3. 動用既有現金", f"${use_normal_cash:,.0f}", delta="優先動用", delta_color="inverse")
else:
...
if use_buffer_cash > 0:
            ...
            st.error("⚠️ 需動用最後防線")
            st.metric("4. 動用抵利型備援", f"${use_buffer_cash:,.0f}", delta="最後防線", delta_color="inverse")
            st.error("⚠️ 需動用備援金")
else:
            st.write("4. 動用抵利型備援: $0 (安全)")
            st.write("4. 動用抵利型備援: $0")
...
# 結餘計算
        ...
        final_balance = (dividend_income + sell_stock_amount + use_normal_cash + use_buffer_cash) - total_expense
...
connector = {"line":{"color":"rgb(63, 63, 63)"}},
            ...
            increasing = {"marker":{"color":"#00CC96"}}, # 綠色 (收入/現金補充)
            totals = {"marker":{"color":"#1f77b4"}}      # 藍色 (小計)
))
st.plotly_chart(fig, use_container_width=True)
        
        if use_buffer_cash == 0:
            st.success("🎉 **資金安全**：不需動用抵利型備援金。")
        else:
            st.warning(f"⚠️ **注意**：已動用既有現金，且需額外提領備援金 **${use_buffer_cash:,.0f}**。")

# --- 資產圖表 ---
st.markdown("---")
@@ -288,4 +304,4 @@ def clean_num(x):
