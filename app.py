...
import plotly.graph_objects as go  # <--- 上次漏掉的就是這行，這次補上了！
import re
...
def parse_my_data(raw_data):
    ...
    for row in raw_data:
        # 防呆：補齊欄位長度，避免 list index out of range
        row = row + [''] * (5 - len(row))
...
# --- 模式 A: 測試模式 (為了確保這次 run 起來沒問題，預設開啟) ---
raw_data_paste = [
...
# --- 模式 B: 正式模式 (連線 Google Sheets) ---
# ⚠️ 確認網頁正常後，請把上面 `raw_data_paste = [...]` 整段刪掉
# 然後把下面這段的註解符號 (#) 拿掉
# -------------------------------------------------------
# import gspread
...
if not df.empty:
    ...
    with c2:
        ...
        if target_withdraw > annual_expense:
            st.success(f"🎉 資金充裕 (盈餘 ${target_withdraw - annual_expense:,.0f})，建議再投資或提高生活品質。")
        else:
            st.warning(f"⚠️ 提領上限 (${target_withdraw:,.0f}) 略低於生活費需求，可能需動用部分備援現金。")
